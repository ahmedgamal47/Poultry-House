...
@section('content')
  <div id="page-wrapper">
    <div class="row">
      <div class="col-lg-12">
        <h1 class="page-header">الشــركات</h1>
      </div>
      <!-- /.col-lg-12 -->
    </div>
    <!-- /.row -->
          ...
          <div class="panel-body">
            <div class="well">
              ...
              <form action="{{ route('admin.company.index') }}" method="get">
                <div class="row">
                  <div class="col-md-3">
                    <input type="text" class="form-control" name="name" placeholder="اسم الشركة"
                           value="{{ old('name') }}">
                  </div>
                  <div class="col-md-3">
                  ...
                  <div class="col-md-2">
                    <select name="categoryId" class="form-control">
                      <option value="" selected>-- مجال الشركة --</option>
            ...
            <table width="100%" class="table table-striped table-bordered table-hover">
              ...
              @foreach($companies as $index => $company)
                <tr class="{{ $index % 2 == 0 ? 'even' : 'odd' }}">
                  <td class="narrow">
                    <img src="{{ $company->user->image->thumbnail->url }}" class="avatar">
                  </td>
                  <td>{{ $company->user->name }}</td>
                  <td>{{ $company->user->phone }}</td>
                  <td>{{ $company->user->address }}</td>
                  <td class="text-center">{{ $company->user->active ? 'مفــعل' : 'غير مفعل' }}</td>
                  <td class="narrow">
                    <div class="text-center">
                      <a href="{{ route('admin.company.show', $company->id) }}" target="_blank" class="btn btn-primary">
                        <i class="fa fa-info-circle"></i>
                      </a>
                      <a href="{{ route('admin.company.edit', $company->id) }}" class="btn btn-info">
